...
countHelper Nothing value = value + 1
countHelper _ value = value
...
slHelper item (sum, length) = (sum + item, length + 1)
...
largestHelper x [] = [x]
largestHelper x xs | x == (head xs) = (x:xs)
                   | x > (head xs) = [x]
                   | otherwise = xs
...
lastHelper value Nothing = Just value
lastHelper _ (Just value) = Just value
