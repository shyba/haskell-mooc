...
whileRight f x = whileHelper f $ f x
  where whileHelper f (Left x) = x
        whileHelper f (Right x) = whileHelper f $ f x
...
joinToLength size xs = [concat | first <- xs, second <- xs, let concat = first ++ second, size == length concat]
...
multiCompose fs x = composeOne (reverse fs) x
    where composeOne [] x = x
          composeOne (ff:ffs) x = composeOne ffs (ff x)
...
multiApp :: ([x] -> b) -> [(c -> x)] -> c -> b
multiApp f xs x = f $ map (\f -> f x) xs
...
interpreter [] = []
interpreter xs = interpret xs 0 0
interpret ("up":xs) x y = interpret xs x (y + 1)
interpret ("down":xs) x y = interpret xs x (y - 1)
interpret ("left":xs) x y = interpret xs (x - 1) y
interpret ("right":xs) x y = interpret xs (x + 1) y
interpret ("printX":xs) x y = show x: interpret xs x y
interpret ("printY":xs) x y = show y: interpret xs x y
interpret _ x y = []
