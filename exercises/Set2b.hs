...
oddOrOne n = if odd n then n else 1
oddFactorial :: Integer -> Integer
oddFactorial 1 = 1
oddFactorial n = (oddOrOne n) * (oddFactorial (n - 1))
...
leftpad str target_len | target_len <= (length str) = str
                       | otherwise = leftpad (" " ++ str) target_len
...
countdownHelper str 0 = str ++ "Liftoff!"
countdownHelper str n = countdownHelper (str ++ (show n) ++ "... ") (n - 1)
...
smallestDivisorHelper n x smallest | x > 1 && n `mod` x == 0 = smallestDivisorHelper n (x - 1) x
                                   | x == 1 = smallest
                                   | otherwise = smallestDivisorHelper n (x - 1) smallest
