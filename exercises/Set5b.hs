...
treeMax Empty = 0
treeMax (Node value left right) = max (max value (treeMax left)) (treeMax right)
...
cull _ Empty = Empty
cull cullValue (Node value left right) | cullValue == value = Empty
                                       | cullValue /= value = (Node value (cull cullValue left) (cull cullValue right))
...
walk [] (Node value left right) = Just value
walk (StepL:xs) (Node value left right) = walk xs left
walk (StepR:xs) (Node value left right) = walk xs right
...
set (StepL:xs) val (Node x left right) = (Node x (set xs val left) right)
set (StepR:xs) val (Node x left right) = (Node x left (set xs val right))
...
search val (Node x left right) | val == x = (Just [])
                               | val /= x = pick (search val left) (search val right)
    where pick Nothing Nothing = Nothing
          pick (Just left) Nothing = (Just (StepL:left))
          pick Nothing (Just right) = (Just (StepR:right))
          pick (Just left) (Just right) = (Just (StepL:left))
