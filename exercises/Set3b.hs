...
buildList start count end = builder start count end []
    where builder start 0 end current = end : current
          builder start 1 end current = start : builder start 0 end current
          builder start count end current = start : builder start (count - 1) end current
...
sums :: Int -> [Int]
sums i = iterate 1 i
    where iterate value total | value == total = [sum (gen value)]
          iterate value total | otherwise = (sum (gen value)):iterate (value + 1) total
          map f [] = []
...
indexDefault xs i def = indexDefaultHelper 0 xs i def
indexDefaultHelper j [] i def = def
indexDefaultHelper j (x:xs) i def | j == i = x
indexDefaultHelper j (x:xs) i def | otherwise = indexDefaultHelper (j + 1) xs i def
...
sorted :: [Int] -> Bool
sorted [] = True
sorted xs = sortedHelper (-100000) xs
sortedHelper :: Int -> [Int] -> Bool
sortedHelper prev [] = True
sortedHelper prev (x:[]) = x >= prev
sortedHelper prev (x:xs) = if x >= prev then sortedHelper x xs else False
...
sumsOfHelper acc (x:[]) = [acc, acc + x]
sumsOfHelper acc (x:xs) = acc: sumsOfHelper (acc + x) xs
...
mymaximum bigger initial (x:xs) = pick x (mymaximum bigger initial xs)
    where pick z w = if bigger z w then z else w
...
maybeMap f [] = []
maybeMap f (x:xs) = pick f (f x) xs
    where pick f (Just value) ys = value:(maybeMap f ys)
          pick f Nothing ys = maybeMap f ys
