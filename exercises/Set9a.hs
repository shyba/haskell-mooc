...
workload nExercises hoursPerExercise = react (nExercises * hoursPerExercise)
  where react hours | hours < 10  = "Piece of cake!"
                    | hours > 100 = "Holy moly!"
                    | otherwise = "Ok."
...
countValid notes = length [x | x <- notes, (length x) >= 6, ((x !! 2) == (x !! 4)) || ((x !! 3) == (x !! 5))]
...
sumSuccess xs = foldl' getdata (Left "no data") xs
  where getdata (Left x) (Left y) = Left x
        getdata (Left x) (Right y) = Right y
        getdata (Right x) (Left y) = Right x
        getdata (Right x) (Right y) = Right (x + y)
...
open attempt (ClosedLock code) | attempt == code = OpenLock code
...
compose [] _ = []
compose ((key, value):as) bs = good (lookup value bs)
  where good Nothing = compose as bs
        good (Just x) = ((key, x):(compose as bs))
...
permute [] x = []
permute ((from, to):as) bs = ((bs !! to):(permute as bs))
